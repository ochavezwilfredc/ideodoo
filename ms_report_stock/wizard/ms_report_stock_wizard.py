...
class MsReportStock(models.TransientModel):
    ...
    datas_fname = fields.Char('Nombre del archivo', readonly=True)
    ...
    def print_excel_report(self):
        ...
        report_name = 'Informe de Stock'
        filename = '%s %s' % (report_name, date_string)

        columns = [
            ('Nro', 5, 'no', 'no'),
            ('Producto', 30, 'char', 'char'),
            ('Categoria de Producto', 20, 'char', 'char'),
            ('Almacen', 30, 'char', 'char'),
            ('Fecha de Entrada', 20, 'datetime', 'char'),
            ('Tiempo en Stock', 20, 'number', 'char'),
        ...
        for column in columns:
            ...
            worksheet.write(row - 1, col, column_name, wbf['header_green'])
        ...
        worksheet.merge_range('A%s:B%s' % (row, row), 'Total General', wbf['total_green'])
        for x in range(len(columns)):
            if x in (0, 1):
                continue
            column_type = columns[x][3]
            if column_type == 'char':
                worksheet.write(row - 1, x, '', wbf['total_green'])
            else:
                if column_type == 'float':
                    wbf_value = wbf['total_float_green']
                else:  # number
                    wbf_value = wbf['total_number_green']
                if x in column_float_number:
                    worksheet.write(row - 1, x, column_float_number[x], wbf_value)
                else:
                    worksheet.write(row - 1, x, 0, wbf_value)

        worksheet.write('A%s' % (row + 2), 'Fecha %s (%s)' % (datetime_string, self.env.user.tz or 'UTC'),
    ...
    def add_workbook_format(self, workbook):
        ...
        wbf['total_float'] = workbook.add_format(
            {'bold': 1, 'bg_color': colors['white_green'], 'align': 'right', 'num_format': '#,##0.00',
             'font_name': 'Georgia'})
        ...
        wbf['total_number'] = workbook.add_format(
            {'align': 'right', 'bg_color': colors['white_green'], 'bold': 1, 'num_format': '#,##0',
             'font_name': 'Georgia'})
        ...
        wbf['total'] = workbook.add_format(
            {'bold': 1, 'bg_color': colors['white_green'], 'align': 'center', 'font_name': 'Georgia'})
        wbf['total'].set_left()
        wbf['total'].set_right()
        wbf['total'].set_top()
        ...
        wbf['total_float_green'] = workbook.add_format(
            {'bold': 1, 'bg_color': colors['green'], 'align': 'right', 'num_format': '#,##0.00',
             'font_name': 'Georgia'})
        wbf['total_float_green'].set_top()
        wbf['total_float_green'].set_bottom()
        wbf['total_float_green'].set_left()
        wbf['total_float_green'].set_right()

        wbf['total_number_green'] = workbook.add_format(
            {'align': 'right', 'bg_color': colors['green'], 'bold': 1, 'num_format': '#,##0', 'font_name': 'Georgia'})
        wbf['total_number_green'].set_top()
        wbf['total_number_green'].set_bottom()
        wbf['total_number_green'].set_left()
        wbf['total_number_green'].set_right()

        wbf['total_green'] = workbook.add_format(
            {'bold': 1, 'bg_color': colors['green'], 'align': 'center', 'font_name': 'Georgia'})
        wbf['total_green'].set_left()
        wbf['total_green'].set_right()
        wbf['total_green'].set_top()
        wbf['total_green'].set_bottom()
